...
def prepare_dataset(in_bed: str) -> Path:
    # Extract dataset name (dataset1 or dataset2) from path
    dataset_name = Path(in_bed).parent.name
    out_dir = Path(f"../../data/intermediate/{dataset_name}/flashzoi_inputs")
    ...
    # Check for duplicate TSS positions
    ...
    seen_genes = set()
    meta_records = []
    for _, r in tqdm(df.iterrows(), total=len(df), desc=f"{dataset_name}"):
        gene = r.gene
    ...
    meta_path = out_dir / "flashzoi_meta.tsv"
